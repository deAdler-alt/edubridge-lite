...
function addWrappedText(doc, text, x, y, maxWidth, lineHeight) {
  ...
  for (const line of lines) {
    doc.text(line, x, y)
    y += lineHeight
    if (y > doc.internal.pageSize.getHeight() - 80) { // więcej bufora na stopkę
      doc.addPage()
      y = 40
    }
  }
...
export async function exportPackToPdf(pack, { title = 'Lite Pack', lang = 'en', appUrl = '' } = {}) {
  ...
  doc.setFontSize(20)
  y = addWrappedText(doc, title, margin, y, pageW - margin * 2, 24)
  y += 6
  ...
  // Easy
  doc.setFont('helvetica', 'bold')
  doc.text(lang === 'pl' ? 'Wersja łatwiejsza' : 'Easy Language', margin, (y += 20))
  doc.setFont('helvetica', 'normal')
  y = addWrappedText(doc, String(pack.easy || ''), margin, y + 6, pageW - margin * 2, 18)

  // Flashcards
  doc.setFont('helvetica', 'bold')
  doc.text('Flashcards', margin, (y += 20))
  doc.setFont('helvetica', 'normal')
